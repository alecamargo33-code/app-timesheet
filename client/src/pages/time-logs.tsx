...
import { useTimeLogs, useCreateTimeLog, useDeleteTimeLog } from "@/hooks/use-time-logs";
...
export default function TimeLogs() {
  ...
  const recentLogs = logs?.slice().sort((a,b) => b.id - a.id).slice(0, 10) || [];

  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="text-3xl font-display font-bold text-foreground">Lançamento de Tempo</h1>
        <p className="text-muted-foreground mt-1">Registre as horas trabalhadas em cada tarefa.</p>
      </div>
          ...
          <CardContent>
            {loadingLogs ? (
              ...
              <div className="space-y-4">
                {recentLogs.map((log) => (
                  <div key={log.id} className="flex items-center justify-between p-4 rounded-xl border border-border/50 bg-card hover:shadow-md transition-shadow group">
                    <div>
                      <div className="font-medium text-foreground">{log.task.name}</div>
                      <div className="text-sm text-muted-foreground flex items-center gap-2 mt-1">
                        <span className="bg-muted px-2 py-0.5 rounded text-xs">{log.user.name}</span>
                        <span>•</span>
