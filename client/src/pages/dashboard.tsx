...
export default function Dashboard() {
  const { data: logs, isLoading: loadingLogs, error: errorLogs } = useTimeLogs();
  ...
  const metrics = useMemo(() => {
    ...
    filteredLogs.forEach(log => {
      ...
      if (isThisWeek(date, { weekStartsOn: 1 })) weekMins += log.durationMinutes;
      ...
      dailyTotals[log.date] = (dailyTotals[log.date] || 0) + log.durationMinutes;
    });
    ...
    const avgMins = Math.round(Object.values(dailyTotals).reduce((a,b) => a+b, 0) / uniqueDays);

    // Prepare chart data
    const pieData = Object.entries(categoryTotals).map(([name, value]) => ({ name, value: Number((value / 60).toFixed(1)) }));
    ...
    return { todayMins, weekMins, topUser, avgMins, pieData, barData, lineData };
  }, [logs, selectedUser]);
  ...
  if (errorLogs) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>Falha ao carregar dados do dashboard.</AlertDescription>
      </Alert>
    );
  }
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Visão geral do tempo investido.</p>
        </div>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card className="shadow-lg shadow-black/5 border-border/50 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              ...
              <CardContent>
                <div className="text-3xl font-bold font-display">{formatDuration(metrics.todayMins)}</div>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-lg shadow-black/5 border-border/50 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              ...
              <CardContent>
                <div className="text-3xl font-bold font-display">{formatDuration(metrics.weekMins)}</div>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-lg shadow-black/5 border-border/50 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              ...
              <CardContent>
                <div className="text-3xl font-bold font-display">{formatDuration(metrics.avgMins)}</div>
              </CardContent>
            </Card>

            <Card className="shadow-lg shadow-black/5 border-border/50 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-sm font-medium text-muted-foreground">Top Usuário</CardTitle>
                <div className="h-10 w-10 bg-amber-500/10 rounded-full flex items-center justify-center">
                  <Trophy className="h-5 w-5 text-amber-500" />
                </div>
              </CardHeader>
              <CardContent>
                <div className="text-xl font-bold truncate" title={metrics.topUser.name}>
                  {selectedUser === 'all' ? metrics.topUser.name : users?.find(u => u.id.toString() === selectedUser)?.name || '-'}
                </div>
                <p className="text-xs text-muted-foreground mt-1">{formatDuration(metrics.topUser.mins)}</p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="shadow-lg shadow-black/5 border-border/50">
              <CardHeader>
                <CardTitle className="font-display">Tempo por Categoria (Horas)</CardTitle>
              </CardHeader>
            ...
            <Card className="shadow-lg shadow-black/5 border-border/50">
              <CardHeader>
                <CardTitle className="font-display">Evolução Diária (Horas)</CardTitle>
              </CardHeader>
              <CardContent className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={metrics.lineData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="hours" stroke="hsl(var(--primary))" strokeWidth={3} dot={{ r: 4, fill: 'hsl(var(--background))', strokeWidth: 2 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
            ...
            <Card className="shadow-lg shadow-black/5 border-border/50 lg:col-span-2">
              <CardHeader>
                <CardTitle className="font-display">Distribuição Total</CardTitle>
              </CardHeader>
