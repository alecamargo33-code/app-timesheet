...
export default function History() {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter(log => {
      const matchUser = filterUser === "all" || log.userId.toString() === filterUser;
      const matchCat = filterCategory === "all" || log.task.category === filterCategory;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        <h1 className="text-3xl font-display font-bold text-foreground">Histórico e Relatórios</h1>
        <p className="text-muted-foreground mt-1">Análise detalhada de todos os lançamentos.</p>
      </div>
      ...
      <div className="flex justify-between items-end">
        <div className="bg-primary/10 text-primary border border-primary/20 px-4 py-2 rounded-lg font-medium flex items-center gap-2">
          ...
          <span className="text-xl font-display font-bold">{formatDuration(totalMinutes)}</span>
        </div>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow><TableCell colSpan={7} className="h-24 text-center text-muted-foreground">Nenhum registro encontrado para os filtros aplicados.</TableCell></TableRow>
              ) : (
                filteredLogs.map((log) => (
                  <TableRow key={log.id} className="hover:bg-muted/50">
                    ...
                    <TableCell className="max-w-[200px] truncate" title={log.task.name}>{log.task.name}</TableCell>
                    <TableCell><Badge variant="secondary" className="font-normal">{log.task.category}</Badge></TableCell>
                    ...
                    <TableCell className="text-right">
                      <Button variant="ghost" size="icon" className="hover:text-destructive" onClick={() => {
                        if (confirm("Excluir este registro?")) deleteMutation.mutate(log.id);
                      }}>
